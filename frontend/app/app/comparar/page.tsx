...
export default function Comparar() {
  ...
  // Histórico de precios de un material (vista de tendencia)
  type PuntoHist = { fecha: string; proveedor: string; precio: number; moneda: string; unidad?: string | null };
  type HistData = { material: { denominacion?: string | null; descripcion?: string | null } | null; puntos: PuntoHist[]; total: number };
  const [histCod, setHistCod]           = useState<string | null>(null);
  const [histMat, setHistMat]           = useState<string>("");
  ...
  async function abrirHistorial(codInt: string, material: string) {
    // Los empalmes (emp_*) no son códigos reales del maestro → sin histórico.
    if (!codInt || codInt.startsWith("emp_")) return;
    setHistCod(codInt); setHistMat(material); setHistData(null); setHistError(null); setHistCargando(true);
    try {
      ...
      setHistError(e instanceof Error ? e.message : "No pudimos cargar el histórico.");
    } finally {
      setHistCargando(false);
    }
  }
  function cerrarHistorial() { setHistCod(null); setHistData(null); setHistError(null); }
  ...
  return (
    ...
    {/* Histórico de precios de un material (tendencia por proveedor) */}
    {histCod !== null && (
      <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center px-4" onClick={cerrarHistorial}>
        <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              <h3 className="text-base font-bold text-[#1A2B4A] leading-tight">{histMat}</h3>
              {histData?.material?.descripcion && <p className="text-xs text-gray-500">{histData.material.descripcion}</p>}
            </div>
            <button onClick={cerrarHistorial} className="text-gray-400 hover:text-gray-700 text-2xl leading-none" aria-label="Cerrar">×</button>
          </div>

          {histCargando && <p className="text-sm text-gray-500 py-12 text-center">Cargando…</p>}
          {histError && <p className="text-sm text-red-600 py-12 text-center">{histError}</p>}
          {histData && !histCargando && !histError && (() => {
            const puntos = histData.puntos.filter((p) => typeof p.precio === "number" && !!p.fecha);
            if (puntos.length === 0) {
                    ...
                    {[0, 0.5, 1].map((f, k) => {
                      ...
                      return (
                        <g key={k}>
                          ...
                          <text x={ml - 8} y={yy + 3} textAnchor="end" fontSize={10} fill="#999">{fmt(val)}</text>
                        </g>
                      );
                    })}
                    {provs.map((pv, pi) => {
                      ...
                      {filasFiltradas.map((row, i) => {
                        ...
                        <tr key={i} className={`border-b border-gray-100 last:border-0 hover:bg-gray-50 ${granDif ? "bg-amber-50/60" : ""}`}>
                          <td className="px-4 py-3">
                            ...
                            {row.cod_int && !row.cod_int.startsWith("emp_") && (
                              <button
                                onClick={() => abrirHistorial(row.cod_int, row.material)}
                                className="mt-1 text-[11px] text-[#E87022] hover:underline font-medium"
                              >
